...
import { department, taskAssign } from '../redux/api/dummyJson';
...
const TaskAssign = () => {
    ...
    const addTask = (type) => {
        navigate("/add-task-assign",{state:{type:type}})
    };
    ...
    return (
        <div className="dashRightView home_section trans">
            <div className='d-flex ac-jb top-search-bar'>
                <h4 className='f3 fs-sm-10 fs-md-16 fs-lg-18 fs-xl-20 fs-xxl-22 mb-0 ps-4 white'>Kitchen Task Assign</h4>
                <div className='d-flex ac-jc '>
                    <button className='add-bar-btn white ms-3' onClick={()=>addTask('add')}>
                        <p className='f2 fs-sm-10 fs-md-10 fs-lg-12 fs-xl-14 fs-xxl-16 mb-0'> + Add Kitchen Assign</p>
                    </button>
                </div>
            </div>
            <Table columns={columns} data={taskAssign} />
        </div>
    )
}
